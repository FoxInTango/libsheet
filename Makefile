...
# ** 项目配置区 **
#    输出文件名称
TARGET_NAME     = libsheet
#    输出文件类型
TARGET_TYPE     = ${TARGET_TYPE_DLL}
#    输出文件后缀 [自动判别]
TARGET_NAME_EXT =
#    安装位置
INSTALL_PATH    = /usr/local/libsheet
...
install :
	rm -rf /usr/local/include/libsheet
	mkdir /usr/local/include/libsheet
	cp  -rf ./include/* /usr/local/include/libsheet
	cp  -rf ./lib/libsheet.so /usr/local/lib
uninstall : 
	rm -rf /usr/local/include/libsheet
	rm -rf /usr/local/lib/libsheet.so
